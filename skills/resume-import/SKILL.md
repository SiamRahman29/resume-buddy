...
3. **Fill the template, do not mechanically convert.** Map the real content (roles,
   dated bullets, education, projects, skills) into the template's macros (`\entry`,
   `points`, sections). Preserve quantified achievements verbatim.
4. Write the result as a `.tex` in the working directory (default `resume.tex`), then
